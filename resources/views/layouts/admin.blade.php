...
<body class="antialiased bg-gray-100 dark:bg-gray-900">
    <div class="flex-col w-full md:flex md:flex-row md:min-h-screen">
        <div @click.away="open = false" class="flex flex-col flex-shrink-0 w-full text-white bg-slate-100 md:w-64 dark:text-gray-200 dark:bg-gray-800" x-data="{ open: false }">
            ...
            <nav :class="{'block': open, 'hidden': !open}" class="flex-grow px-4 pb-4 md:block md:pb-0 md:overflow-y-auto text-white">
...
{{--            @if (session()->has('danger'))--}}
                <div class="p-4 mb-4 text-sm text-red-700 bg-red-600 rounded-lg dark:bg-red-200 dark:text-red-800"
                     role="alert">
                    <span class="font-medium">{{ session()->get('danger') }}</span>
                </div>
{{--            @endif--}}
            @if (session()->has('success'))
                <div class="p-4 mb-4 text-sm text-green-700 bg-green-500 rounded-lg dark:bg-green-200 dark:text-green-800"
                     role="alert">
                    <span class="font-medium">{{ session()->get('success') }}</span>
                </div>
            @endif
            @if (session()->has('warning'))
                <div class="p-4 mb-4 text-sm text-yellow-700 bg-yellow-400 rounded-lg dark:bg-yellow-200 dark:text-yellow-800"
                     role="alert">
